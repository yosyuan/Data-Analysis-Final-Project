# -*- coding: utf-8 -*-
"""02_Temple Density (km^2) and Marriage Rate line chart
...
r_value, p_value = pearsonr(df['Marriage Rate'], df['Temple Density (km^2)'])

# Create horizontal line plot
plt.figure(figsize=(12, 6))
plt.plot(df["City"], df["Temple Density (km^2)"], marker='o', label="Temples")
plt.plot(df["City"], df["Marriage Rate"], marker='s', label="Marriage Rate")

# Optimize layout
plt.xticks(rotation=45, ha='right')
plt.xlabel("City")
plt.ylabel("Density")
plt.title(f"Temples and Marriage Rate by City\nr = {r_value:.2f}, p = {p_value:.3f}")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()